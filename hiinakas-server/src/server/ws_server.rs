...
type EventHandler = Arc<
    dyn (Fn(String, Vec<u8>) -> futures_util::future::BoxFuture<'static, ()>) + Send + Sync
>;
...
impl WebSocketServer {
    ...
    pub async fn emit(&self, event: EventType, data: Vec<u8>) -> Result<(), Box<dyn std::error::Error>> {
        let msg = WsEvent {
            event: event.into(),
            data,
        };
                                ...
                                match ws_sender.send(message).await {
                                    ...
                                    Err(e) => {
                                        if let WsMessage::Binary(data) = message_clone {
                                            ...
                                            error!("Failed to send message to {}: {:?}, message: {:?}", connection_id, e, message_proto);
                                        }
                                        break;
                                    }
                                }
                                ...
                                match msg_result {
                                    Ok(msg) => {
                                        if let WsMessage::Binary(data) = msg {
                                            server_clone.handle_event(&connection_id_clone, data.to_vec()).await;
                                        }
                                    }
                                    Err(e) => {
        ...
        match WsEvent::decode(&data[..]) {
            Ok(event) => {
                ...
                if event_type == EventType::Unknown {
                    error!("Unknown event type: {:?}, with data: {:?}", event.event(), event.data);
                    return;
                }
    ...
    async fn handle_disconnect(&self, connection_id: &str) {
        ...
        trace!("Disconnected connection {} from all rooms and connections", connection_id);
    }
}
...
impl ConnectionEmitter {
    pub async fn emit(&self, event: EventType, data: Vec<u8>) -> Result<(), Box<dyn std::error::Error>> {
        ...
        if let Some(conn) = self.server.connections.read().await.get(&self.connection_id) {
